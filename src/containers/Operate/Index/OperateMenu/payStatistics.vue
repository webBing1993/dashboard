  ...
  <el-main>
    <div class="module-wrapper">
      ...
      <el-row class="el_row">
        ...
        <el-col :span="4" class="el_col_left">
          <div>
      ...
      <div class="v-table">
        <table border="0">
          ...
          <tr v-for="(obj, index) of tableData">
            <td>{{ index+1}}</td>
            <td>{{ obj.orderType}}</td>
            <td>{{ obj.pmsOrderNo }}</td>
            <td>{{ obj.totalFee }}</td>
            <td>{{ obj.payFlag| payFlagFilter}}{{ obj.tradeType=='refund'?'退款':(obj.tradeType!=''?'支付':'')}}</td>
            <td>{{ obj.roomNo }}</td>
            <td>{{ obj.guestName }}</td>
            <td>{{ obj.inTime }}</td>
            <td>{{ obj.orderId }}</td>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...mapActions([
        'getHotelPaySta','getHotelListSta','showtoast'
      ]),
      initHotelList(){
